 ...
 */
  ...
  formatMoney: (
    ...
    return CurrencyFormatter.formatMoneyAsObject(
      number,
      symbol,
      decimalDigits,
      thousandSeparator,
  ...
  formatNumber: (
     ...
     */
    number = CurrencyFormatter.unformat(number);
    /**
    ...
    const opts = CurrencyFormatter.prepareOptions(toPrepare);
    ...
    const base =
      parseInt(
        CurrencyFormatter.toFixed(Math.abs(number || 0), usePrecision),
        10
      ) + '';
    ...
    if (usePrecision) {
      const fNum = String(
        parseFloat(CurrencyFormatter.toFixed(Math.abs(number), usePrecision)) ||
          0
      );
      if (fNum.includes('.')) {
  ...
  formatMoneyAsObject: (
    ...
    const formats = CurrencyFormatter.checkCurrencyFormat(
    ...
    const formattedNumber = CurrencyFormatter.formatNumber(
      Math.abs(number),
 ...
 */
  prepareOptions: function prepareOptions(options?: Currency): Currency {
    /**
    ...
    if (isNonNullString(object.format)) {
      const p = CurrencyFormatter.parseFormat(object.format);
      if (p.format) {
   ...
   */
  checkCurrencyFormat: function checkCurrencyFormat(
    format:
      | string
